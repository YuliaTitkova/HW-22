...
const counter = (function() {
    let count = 0; // внутренний счетчик, доступный только через замыкание

    return function(n) {
        if (n !== undefined) { // если передано число, начинаем счет с него
            count = n;
        }
        return count++; // возвращаем текущее значение и увеличиваем на 1
    };
})();
...
const counterFactory = (function() {
    ...
    return {
        value(n) {
            if (n !== undefined) {
                count = n; // устанавливаем новое значение, если передано
            }
...
console.log(myDouble(3)); // 6 = myMul(2, 3)
console.log(myDouble(4)); // 8 = myMul(2, 4)
console.log(myDouble(5)); // 10 = myMul(2, 5)

console.log(myTriple(3)); // 9 = myMul(3, 3)
console.log(myTriple(4)); // 12 = myMul(3, 4)
console.log(myTriple(5)); // 15 = myMul(3, 5)
